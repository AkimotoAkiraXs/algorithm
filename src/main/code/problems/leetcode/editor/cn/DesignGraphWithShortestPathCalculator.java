...
public class DesignGraphWithShortestPathCalculator {
    ...
    class Graph {
        ...
        public Graph(int n, int[][] edges) {
            this.n = n;
            g = new int[n][n];
            for (int[] gg : g) Arrays.fill(gg, INF);
        ...
        public int shortestPath(int node1, int node2) {
            ...
            while (true) {
                int x = -1;
                for (int i = 0; i < n; i++)
                    if (!f[i] && (x < 0 || dis[i] < dis[x])) x = i;
                if (x < 0 || dis[x] == INF) return -1;
                if (x == node2) return dis[x];
                f[x] = true;
                for (int i = 0; i < n; i++) dis[i] = Math.min(dis[i], dis[x] + g[x][i]);
            }
        }
    }
